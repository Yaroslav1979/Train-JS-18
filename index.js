...
function getAge(age) {
  ...
  try { 
    ...
    return `Вік користувача: ${age} років`;
   }  
    catch(error) {      
...
function getBookById(books, id) {
  try {
    ...
    // якщо return book , то виводить об'єкт { id: 2, title: "Книга 2" }
  }
  catch(error) {      
...
function decodeURIComponentWrapper(encodedString) {
  try {
    const decodURI = decodeURI(encodedString);
      return decodURI;
    }        
     catch(error) {
...
function findEvenNumber(numbers) {
      let evenNumber;
      try { 
        if (evenNumber = numbers.find(evenNumber => evenNumber % 2 === 0)) {          
            return evenNumber          
        }  
...
function validateUser(user) {
  ...
  if (user === undefined) {
    ...
    user.name === undefined) {
      throw new Error("Ім'я користувача не вказано!", {case: user});
      } else if (  
    user.email === undefined) {
      throw new Error("Email користувача не вказано!", {case: user});
   }
  }
  catch (error) {
